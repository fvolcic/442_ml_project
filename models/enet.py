...
class FirstBlock(nn.Module):
    def __init__(self, in_channels=3, out_channels=13):
        ...
        self.threeXthree = nn.Conv2d(in_channels = in_channels, out_channels = out_channels, kernel_size = 3, stride=2, padding=1)
...
class bottleneck_sample(nn.Module):
    ...
    def forward(self, input, indices=None, size=None):
        ...
        #print("proj",right_result.shape)
        if self.upsample:
            right_result = self.middle(right_result)
            #print("middle", right_result.shape)
        else:
            right_result = self.middle(right_result)
        ...
        if self.downsample:
            ...
            # print(indices.shape)
            # print(input.shape)
            # if hold.size() != indices.size():
            #     pads = (indices.size(3) - hold.size(3), 0, indices.size(2) - hold.size(2), 0)
            #     hold = nn.functional.pad(hold, pads, "constant", 0)
            # print("Post-Manip")
            # print(indices.shape)
            # print(hold.shape)
